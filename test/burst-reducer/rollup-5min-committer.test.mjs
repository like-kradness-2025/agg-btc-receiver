  ...
  it('reconciles committed 30s windows into missing 5min rows', () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const rows = [{
        ...
        trade_count_30s: (i + 1) * 30,
        buy_trade_count_30s: i * 30,
        sell_trade_count_30s: 30,
        traded_qty_30s: (i + 1) * 60,
        traded_notional_30s: (i + 1) * 300,
        buy_qty_30s: (i + 1) * 30,
        sell_qty_30s: (i + 1) * 30,
        buy_notional_30s: (i + 1) * 180,
        sell_notional_30s: (i + 1) * 120,
        signed_volume_30s: 0,
        signed_notional_30s: (i + 1) * 60,
        max_trade_notional_30s: (i + 1) * 10,
        large_trade_count_30s: i % 2,
        large_trade_notional_30s: (i % 2) * (i + 1) * 20,
        _quality: {
          source_layer: 'features_30s',
