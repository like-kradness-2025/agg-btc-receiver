...
function makeRows({ start = START, market = MARKET, empty = false } = {}) {
  return Array.from({ length: 30 }, (_, i) => ({
    ...
    trade_open_1s: empty ? null : 1000 + i,
    trade_high_1s: empty ? null : 1000 + i + 0.5,
    trade_low_1s: empty ? null : 999.5 + i,
    trade_close_1s: empty ? null : 1000 + i + 0.25,
    trade_count_1s: empty ? 0 : i + 1,
    buy_trade_count_1s: empty ? 0 : i,
    sell_trade_count_1s: empty ? 0 : 1,
    traded_qty_1s: empty ? 0 : (i + 1) * 2,
    traded_notional_1s: empty ? 0 : (i + 1) * 10,
    buy_qty_1s: empty ? 0 : i + 1,
    sell_qty_1s: empty ? 0 : i + 1,
    buy_notional_1s: empty ? 0 : (i + 1) * 6,
    sell_notional_1s: empty ? 0 : (i + 1) * 4,
    signed_volume_1s: empty ? 0 : 0,
    signed_notional_1s: empty ? 0 : (i + 1) * 2,
    max_trade_notional_1s: empty ? 0 : (i + 1) * 10,
    large_trade_count_1s: empty ? 0 : (i % 2),
    large_trade_notional_1s: empty ? 0 : (i % 2) * (i + 1) * 10,
    _quality: {
...
describe('C1 pure 30s rollup', () => {
  it('exposes the approved P2 rollup-only field/operator matrix', () => {
    ...
    large_trade_notional_share_30s: 'recompute',
      ...
      ofi_30s: 'sum', bid_add_qty_30s: 'sum', bid_cancel_qty_30s: 'sum',
      ask_add_qty_30s: 'sum', ask_cancel_qty_30s: 'sum',
      replenishment_qty_30s: 'sum', pulling_qty_30s: 'sum',
    });
  ...
  it('computes hand-derived mean, max, sum, and nearest-rank p95 values', () => {
    ...
    assert.equal(result.trade_open_30s, 1000);
    assert.equal(result.trade_high_30s, 1029.5);
    assert.equal(result.trade_low_30s, 999.5);
    assert.equal(result.trade_close_30s, 1029.25);
  });
  ...
  it('returns only one 30s row and has no 5min implementation in C1', () => {
    ...
    assert.deepEqual(Object.keys(output[0]).sort(), [
      '_quality',
      'ask_add_qty_30s', 'ask_cancel_qty_30s', 'bid_add_qty_30s', 'bid_cancel_qty_30s',
      'burst_count_max_30s',
      ...
      'signed_volume_30s',
      'signed_notional_30s',
      'max_trade_notional_30s',
      'large_trade_count_30s',
      'large_trade_notional_30s',
      'trade_imbalance_qty_30s',
      'trade_imbalance_notional_30s',
      'mean_trade_notional_30s',
      'large_trade_notional_share_30s',
      'trade_open_30s',
      'trade_high_30s',
      'trade_low_30s',
      'trade_close_30s',
      'ofi_30s', 'pulling_qty_30s', 'replenishment_qty_30s',
