...
## C11. 訂正（Astra 監査 2026-09-25 の指摘を決定として反映）

- **C8 の文言を訂正する。** 「raw の重複排除（applied_boundary まで）」は誤り。**raw の耐久位置**と
  **板の適用位置**は別の名前・別の所有者で持つ。raw の重複排除は raw の耐久位置まで、板への配送は
  「耐久化済みだが未適用のもの」を対象にし、再起動時は**板の適用位置から**再送する。
- **C3 の「Binance は ack を返さない」は誤り。** Binance Spot の動的 SUBSCRIBE は `{"result":null,"id":…}` を返す。
  ack 方式は venue 単位ではなく**接続・購読方式ごと**に宣言する（`explicit` / `first-data` の二択を接続単位で決める）。
  「既に購読済み」を成立に変換する場合も、**対象購読への対応**を確認してからにする。
- **C2 に run 交代の認可規則を追加する。** 一意なIDだけでは新旧は決まらない。世代の大小は**同じ run の中**でのみ
  意味を持つ。別 run への交代は、**「この接続を引き継ぐ」と明示された場合のみ**認め、旧 run の再侵入は拒否する。
- **C6 は「板の整合」と「履歴の完全性」を分離する。** 板の整合は snapshot と差分の接続条件で示し、
  履歴の完全性は raw 側の記録で示す。Kraken の CRC32 は**上位10水準のみ**が対象で、全深度の証明にはならない。
  `data_complete` は**対象 stream・期間・検証範囲**に結び付けて主張する。
- **C7 の穴の扱いを訂正する。** 旧世代の穴の存在を理由に**新しい接続の accept を禁止してはいけない**。
  現在の板は**新しい snapshot で回復**させ、過去の欠測は**期間別に記録として残す**（回復と記録を混ぜない）。
- **C4 に pong の識別と応答期限を追加する。** 生存判定は「最後の pong からの経過」で行い、
  市場更新の停滞判定とは分離する。採用する WS 実装の**自動 pong と ping 観測の能力**も契約条件として固定する。
- **Bitfinex アダプタは当面使用禁止。** `chanId` と購読種別の対応が無く、`[chanId,[price,count,amount]]` を
  `parse()` が `null` に落とし、約定スナップショットを板と誤判定し、trade ID を価格として扱う（監査で再現済み）。
  移植の際は v2 公式仕様（`chanId` 管理・板と約定の分離）で**書き直す**まで実運用に載せない。
- **プロセス分離は未実証。** 現状の `structure` は同期呼び出しの単一プロセス構成。3プロセス化と障害訓練
  （受信再起動 / spool 溢れ / 板の単独再起動）は未実施であり、完了条件に含める。
- **進め方を訂正する。** 「共通部分を全部閉じてから venue」ではなく、
  ①契約訂正 → ②接続交代・再配送・readiness を一体で検証 → ③**Kraken v2 を snapshot/checksum まで通す**
  → ④実3プロセスで障害試験 → ⑤他 venue 展開 の順とする。
