...
export class BinanceCoinmPerpConnector extends BinancePerpConnector {
  ...
  async _syncBook() {
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
      try {
        ...
        // Discard the sequence from this failed snapshot attempt before the
        // next REST retry; retaining it can bridge against the wrong snapshot.
        this._ringBuf = [];
        this._ringBufPos = 0;
   ...
   *  R-11: the same silent-drop hole as the Binance spot/perp overrides — a
   *  zero/NaN price or contract count is counted by _emitTrade's fail-closed
   *  guard (qty becomes 0/Infinity/NaN, all rejected) instead of vanishing. */
  _handleTrade(event) {
