...
const normalizeTs = (value) => {
  ...
  // never substitute the local receive time. Callers fail-closed on null.
  if (value == null) return null;
...
// BitMEX orderBookL2 frames carry no exchange event timestamp: ts is the
// local processing time. Issue #12 marks that explicitly so
// source_event_ts_ms never inherits a locally synthesized wall clock.
