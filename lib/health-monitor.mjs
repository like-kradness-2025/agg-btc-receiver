...
// 正常時の生存行の間隔。監視 (scripts/receiver-auto-restart-watchdog.py) は
// health.jsonl の最終行が 90 秒より古いと再起動をかけるため、完全に 0 には
// できない。異常時はこの値に関係なく毎 tick (1 秒) 書く。
export const HEALTH_HEALTHY_WRITE_INTERVAL_MS = 30_000;
// 「静かな市場」を検知して書く閾値。depth/trade がこの時間届かない市場があれば
// 生存行の間隔を待たずに書く (監視の depth_stale:90s を鈍らせないため)。
export const HEALTH_STALE_MARKET_WRITE_MS = 60_000;

/** 行が異常 (state/degraded/ioFailure/loss/completeness) を含むか。 */
function rowIsAnomaly(report) {
  if (report.state !== 'normal') return true;
  if (report.data_complete !== true) return true;
  if ((report.raw_db_dropped_events ?? 0) > 0) return true;
  if ((report.raw_db_pending_overflow_events ?? 0) > 0) return true;
  if ((report.raw_db_post_drain_dropped_events ?? 0) > 0) return true;
  for (const market of Object.values(report.markets ?? {})) {
    if (market.state !== 'running' || market.ioFailure) return true;
  }
  return false;
}

/** 状態の変化検出用の軽量シグネチャ (カウンタは含めない = 毎 tick 変化するため)。 */
function rowSignature(report) {
  const markets = Object.entries(report.markets ?? {})
    .map(([name, market]) => [name, market.state ?? null, market.degraded_reason ?? null,
      market.ioFailure ? market.ioFailure.count : 0])
    .sort((a, b) => (a[0] < b[0] ? -1 : 1));
  return JSON.stringify({
    state: report.state,
    data_complete: report.data_complete ?? null,
    degraded: Object.keys(report.degraded_markets ?? {}).sort(),
    markets,
  });
}

/** depth/trade が閾値以上届いていない市場があるか (静かな市場の検知)。 */
function hasStaleMarket(report, nowMs, staleMs) {
  if (!(staleMs > 0)) return false;
  for (const market of Object.values(report.markets ?? {})) {
    for (const at of [market.lastDepthMsgAt, market.lastTradeMsgAt]) {
      if (Number.isFinite(at) && nowMs - at > staleMs) return true;
    }
  }
  return false;
}
...
export class HealthMonitor {
  ...
  constructor(outputPath, options = {}) {
    ...
    // 0 = 従来どおり毎 tick 書く (テスト・互換用)。
    this._healthyWriteIntervalMs = options.healthyWriteIntervalMs ?? HEALTH_HEALTHY_WRITE_INTERVAL_MS;
    this._staleMarketWriteMs = options.staleMarketWriteMs ?? HEALTH_STALE_MARKET_WRITE_MS;
  ...
  /**
   * 記録ポリシー: 異常時は毎 tick (1 秒)、正常時は状態が変わった時と
   * healthyWriteIntervalMs ごとの生存行だけ書く。監視 (watchdog) は最終行の
   * 鮮度 90 秒で再起動を判断するため、生存行はその内側で維持する。
   */
  _shouldWrite(report, nowMs, transitionsChanged) {
    if (transitionsChanged) return true;
    if (rowIsAnomaly(report)) return true;
    if (hasStaleMarket(report, nowMs, this._staleMarketWriteMs)) return true;
    if (this._healthyWriteIntervalMs <= 0) return true;
    if (rowSignature(report) !== this._lastSignature) return true;
    return nowMs - this._lastWriteAtMs >= this._healthyWriteIntervalMs;
  }
  ...
  _tick() {
    ...
    const transitionsChanged = transitionsKey !== this._lastTransitionsKey;
    if (!this._shouldWrite(report, nowMs, transitionsChanged)) {
      // スキップした tick では遷移リストを消費しない (書けなかった遷移を失わない)。
      return;
    }
    if (transitionsChanged) this._lastTransitionsKey = transitionsKey;
    else report.completeness_transitions = [];
    this._lastWriteAtMs = nowMs;
    this._lastSignature = rowSignature(report);
