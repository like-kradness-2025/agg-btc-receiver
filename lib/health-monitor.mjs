...
export class HealthMonitor {
  ...
  async close() {
    ...
    // R-13: the shutdown drain (and therefore the counted drop) happens AFTER
    // the last periodic tick, so a noted drop needs one explicit final row to
    // be durable in health.jsonl. Only emitted when something was dropped —
    // the normal path keeps its previous row counts.
    if (this._rawDbDrop) {
      try {
