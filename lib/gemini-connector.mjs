...
export class GeminiConnector extends TradeOnlyConnector {
  ...
  async _connectDepthWs() {
    ...
    this._depthWs.on('message', (raw) => {
      // Issue #12: the depth WS is a separate socket from the main WS, so
      // capture its ingress metadata at this socket's own message boundary —
      // this._ingress only tracks the main socket's frames.
      const ingress = this._captureIngress();
  ...
  _handleDepthUpdate(data, ingress = null) {
    ...
    for (const t of tradeRows) {
      ...
      if (rawTradeTs != null) {
        ...
        if (Number.isFinite(num)) tradeTs = num;
        else {
          const parsed = Date.parse(rawTradeTs);
          if (Number.isFinite(parsed)) tradeTs = parsed;
        }
      }
