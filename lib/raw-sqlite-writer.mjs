...
class AppendTrace {
  ...
  summary() {
    ...
    if (this.gcObserver) { try { this.gcObserver.disconnect(); } catch {} }
    return {
  ...
  append(envelopes) {
    ...
    const run = () => {
      ...
      const report = (outcome) => {
        // slowAppendMs < 0 で無効化。0 は毎回報告 (検証用)。
        if (trace === null || this.slowAppendMs < 0) return;
        const totalMs = trace.totalMs();
        if (totalMs < this.slowAppendMs) return;
