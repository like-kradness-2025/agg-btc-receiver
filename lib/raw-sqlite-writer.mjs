...
function lineSortKey(line) {
  const tsMatch = /"event_ts_ms":(-?\d+)/.exec(line);
  const seqMatch = /"ingest_seq":"?(\d+)"?/.exec(line);
  return {
...
export function mergeLinesIntoBatch(meta, incoming) {
  ...
  for (const line of lines) {
    const m = /"recv_ts_ms":(-?\d+)/.exec(line);
    if (m) recvTimes.push(Number(m[1]));
  }
...
export class RawSqliteWriter {
  ...
  append(envelopes) {
    ...
    const madeBatches = freshGroups.map(makeBatch);
    this.queue = this.queue.then(() => this._insertBatches(madeBatches, lateGroups));
    return this.queue;
  }
  ...
  async _backfillLateGroup({ market, stream, rows }) {
    ...
    await this._withMarketDb(market, async ({ db }) => {
      ...
      // 対象候補: [minTs, maxTs] と重なる batch (時間範囲検索・インデックス使用)
      ...
      // 各行の挿入先を決める (候補は時間で非重叠が保証されていないため全照合)
      const byBatch = new Map(); // batch_id -> Array<{row, index}>
      const leftovers = [];
      for (const row of rows) {
        const ts = eventOrderKey(row);
        if (ts === Number.MAX_SAFE_INTEGER) { leftovers.push(row); continue; }
        const target = candidates.find((c) => ts >= Number(c.first_event_ts_ms)
      ...
      const stats = { mergedBatches: 0, mergedRows: 0 };
      await this._runWithBusyRetry(db, () => {
        db.exec('BEGIN IMMEDIATE');
        try {
          ...
          if (leftovers.length > 0) {
            ...
            `);
            // leftovers も event 順に並べた mini-batch として登録する
            for (const row of sortBatchRows(leftovers)) {
              const batch = makeBatch([row]);
              insert.run(
                batch.schema, batch.market, batch.stream,
                batch.first_event_ts_ms, batch.last_event_ts_ms,
                batch.first_recv_ts_ms, batch.last_recv_ts_ms, batch.row_count,
