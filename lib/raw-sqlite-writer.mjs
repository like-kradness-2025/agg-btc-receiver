...
export class RawSqliteWriter {
  constructor({
    ...
    this.freshDedupeStats = { skipped: 0, lastSkippedAtMs: null };
  }
  ...
  /**
   * R-02: fresh 経路で「コミット済み identity の再送」を棄却する。
   * 窓が空ならそのまま返す (通常運転のコストをゼロに保つ)。
   */
  _dropFreshRedeliveries(key, rows) {
    if (this.freshDedupe.size === 0) return rows;
    const [market, stream] = key.split('\u0000');
    let skipped = 0;
    const kept = [];
    ...
    console.error(
      `[RawSqliteWriter] ${market}/${stream}: ${skipped} re-delivered row(s) skipped on the fresh path`
      + ` (already committed; R-02) [total skipped=${this.freshDedupeStats.skipped}]`,
    );
    return kept;
  }
  ...
  append(envelopes) {
    ...
    const run = () => {
      ...
      return this._insertBatches(freshGroups.map(makeBatch), lateGroups).then((result) => {
