...
// normalizes, reorders or interprets exchange payloads. The frame is wrapped
// in an additive envelope whose top-level keys mirror the legacy
// raw_v6_sqlite line shape so existing raw parsers stay compatible.
export function normalizeCanonicalEnvelope(envelope, nowMs, writerSessionId, ingestSeq) {
  ...
  if (frameText.includes('\n')) {
    throw new TypeError('canonical envelope frame_text must not contain newlines');
  }
...
export class RawSqliteWriter {
  ...
  _bumpLateStats(key, field, amount = 1) {
    if (!this.lateEventStats.has(key)) {
      this.lateEventStats.set(key, { merged: 0, mergedRows: 0, minibatches: 0, overlaps: 0 });
    }
  ...
  async _backfillLateGroup({ market, stream, rows }) {
    ...
    await this._withMarketDb(market, async ({ db }) => {
      ...
      const stats = { mergedBatches: 0, mergedRows: 0, minibatches: 0, overlaps: 0 };
      await this._runWithBusyRetry(db, () => {
        db.exec('BEGIN IMMEDIATE');
        try {
  ...
  pruneExpired(nowMs = this.now()) {
    ...
    this.queue = this.queue.then(() => {
      for (const { db } of this.databases.values()) {
        ...
        // Canonical log rows are immutable until operator TTL pruning removes
        // expired arrivals; the write path itself never deletes or updates.
        db.prepare('DELETE FROM canonical_frames WHERE recv_ts_ms < ?').run(cutoff);
