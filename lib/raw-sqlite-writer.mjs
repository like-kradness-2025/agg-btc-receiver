...
const GC_ENTRY_SUPPORTED = (() => {
  try { return PerformanceObserver.supportedEntryTypes.includes('gc'); } catch { return false; }
})();
...
class AppendTrace {
  ...
  summary() {
    ...
    return {
      ...
      gc_supported: GC_ENTRY_SUPPORTED,
...
export class RawSqliteWriter {
   ...
   */
  _rememberFreshIdentities(key, rows, committedAtMs, { sweep = true } = {}) {
    const [market, stream] = key.split('\u0000');
    let identityChars = 0;
    for (const row of rows) {
      const identity = freshSourceIdentity(market, stream, row);
    ...
    // 呼び出し側が append 単位でまとめて sweep する場合は省略できる
    // (2026-09-22 実測: グループ毎の sweep が 28 回走り、最大131,072キーの
    //  走査と Map の churn で writer.remember_fresh が 1.27〜1.45 秒を占めていた)。
    if (sweep) this._sweepFreshDedupe(committedAtMs);
    return identityChars;
  }

  /** TTL と件数上限の両方で窓を有界に保つ (挿入順 = 古い順に落とす)。 */
  _sweepFreshDedupe(nowMs) {
    if (this.freshDedupe.size === 0) return;
    const cutoff = nowMs - this.freshDedupeWindowMs;
    // 反復中に delete すると V8 側で走査コストが跳ね上がる (実測 833ms/append)。
    // 失効分は一度集めてから、反復が終わって削除する。
    let expired = null;
    for (const [identity, at] of this.freshDedupe) {
      if (at < cutoff) (expired ??= []).push(identity);
    }
    if (expired !== null) for (const identity of expired) this.freshDedupe.delete(identity);
    // 上限の追い出しも同様に、対象を集めてから削除する (挿入順=古い方から)。
    const overflow = this.freshDedupe.size - this.freshDedupeMaxKeys;
    if (overflow > 0) {
      const doomed = new Array(overflow);
      let n = 0;
      for (const identity of this.freshDedupe.keys()) {
        doomed[n++] = identity;
        if (n === overflow) break;
      }
      for (let k = 0; k < n; k += 1) this.freshDedupe.delete(doomed[k]);
    }
  }
  ...
  append(envelopes) {
    ...
    const run = () => {
      ...
      return this._insertBatches(freshBatches, lateGroups, trace).then((result) => {
        ...
        const committedRows = committed.reduce((sum, group) => sum + group.rows.length, 0);
        const endIdentity = trace?.begin('writer.remember_identity', { rows: committedRows });
        let identityChars = 0;
        for (const group of committed) {
          identityChars += this._rememberFreshIdentities(group.key, group.rows, committedAtMs, { sweep: false });
        }
        endIdentity?.({ identity_chars: identityChars, rows: committedRows });
        const endSweep = trace?.begin('writer.remember_sweep', { window_keys: this.freshDedupe.size });
        this._sweepFreshDedupe(committedAtMs); // append につき1回だけ
        endSweep?.();
        endRemember?.({ groups: committed.length, identity_chars: identityChars, rows: committedRows });
        report('ok');
        return result;
      }, (error) => {
