...
export class BaseConnector extends EventEmitter {
        ...
        try {
          ...
          this._ws.on('message', (raw) => {
            ...
            try {
              ...
              // Issue #10: capture the exact source frame BEFORE any semantic
              // parse / normalization. Opt-in per connector via
              // _canonicalFrameMeta(); the frame keeps ingress metadata and
              // the byte-exact message text and is never interpreted here.
              const frame = this._makeCanonicalFrame(data, rawText);
  ...
  /**
   * Opt-in frame classifier. Connectors that expose self-describing frames
   ...
   * @returns {Object|null}
   */
  _makeCanonicalFrame(data, rawText) {
    const meta = this._canonicalFrameMeta(data);
