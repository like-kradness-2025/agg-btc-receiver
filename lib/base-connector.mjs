...
export class BaseConnector extends EventEmitter {
      ...
      (async () => {
        try {
          ...
          this._ws.on('close', (code, reason) => {
            ...
            console.error(`[${this.market}] WS CLOSE code=${code} reason=${reason ? reason.toString() : '(empty)'} settled=${settled} state=${this._state} gen=${generation}/${this._wsGeneration} ${this._liveness.text()} ${this._stats.channelSilence}`);
  ...
  /** @returns {Object} */
  getStats() {
    return { ...this._stats, state: this._state, pongAgeMs: this._liveness.pongAgeMs() };
  }
    ...
    this._staleTimer = setInterval(() => {
      if (['running', 'connected', 'syncing'].includes(this._state)) {
        ...
        if (elapsed > STALE_MSG_THRESHOLD_MS) {
          this.emit('error', { market: this.market, message: `no message for ${elapsed}ms, reconnecting (${this._liveness.text()} ${this._stats.channelSilence})` });
          this._clearPingTimer();
