      ...
      (async () => {
        try {
          ...
          this._ws.on('close', (code, reason) => {
            ...
            console.error(`[${this.market}] WS CLOSE code=${code} reason=${reason ? reason.toString() : '(empty)'} settled=${settled} state=${this._state} gen=${generation}/${this._wsGeneration} ${this._liveness.text()}`);
    ...
    this._staleTimer = setInterval(() => {
      if (['running', 'connected', 'syncing'].includes(this._state)) {
        ...
        if (elapsed > STALE_MSG_THRESHOLD_MS) {
          this.emit('error', { market: this.market, message: `no message for ${elapsed}ms, reconnecting (${this._liveness.text()})` });
          this._clearPingTimer();
