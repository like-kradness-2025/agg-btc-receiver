 ...
 *
 * The canonical layer is deliberately TTL-exempt (see RawSqliteWriter) so it
 * grows without bound; on a capacity-constrained host the operator turns the
 * copy off instead of letting it fill the disk. Nothing downstream reads the
 * table (agg-btc-downstream parses `raw_batches`), so disabling it costs no
 * consumer. Disabled with RECEIVER_CANONICAL_RAW=0|false; anything else keeps
 * the historical behavior (enabled).
 */
export const CANONICAL_RAW_DISABLED_VALUES = ['0', 'false'];
export const CANONICAL_RAW_ENABLED_VALUES = ['1', 'true'];

export function resolveCanonicalRawEnabled(env = process.env) {
  const raw = env?.RECEIVER_CANONICAL_RAW;
  if (raw === undefined || raw === null || raw === '') return true;
  if (CANONICAL_RAW_DISABLED_VALUES.includes(raw)) return false;
  if (CANONICAL_RAW_ENABLED_VALUES.includes(raw)) return true;
  console.error(
    `[raw-db-pending] ignoring invalid RECEIVER_CANONICAL_RAW=${raw} `
    + '(expected 0|1|false|true); keeping the canonical raw copy enabled',
  );
  return true;
}
