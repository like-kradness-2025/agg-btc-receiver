...
export class StallProbe {
  ...
  start() {
    ...
    // GC 停止の計測 (0.5秒以上のみ記録)。単一workerの停止の最有力候補 (Astra 監査)。
    // 復帰後に通知されるので、停滞中の照会に頼らない形になっている。
    this._gcPauses = this._gcPauses ?? [];
    void import('node:perf_hooks').then(({ PerformanceObserver }) => {
      try {
          ...
          try {
            for (const e of list.getEntries()) {
              if (e.duration >= 500) {
                ...
                this._gcPauses.push({ at: Date.now(), dur_ms: dur, kind: e.detail?.kind ?? 'gc' });
                if (this._gcPauses.length > 20) this._gcPauses.shift();
                // 2秒以上のGC停止は「そのまま停滞の原因候補」なので即時に1行だけ記録する
                // (異常時のみ／通常のGCは記録しない)。
                if (dur >= 2000) {
                  console.error(`[gc] long pause: ${dur}ms kind=${e.detail?.kind ?? 'gc'}`);
                }
              }
            }
          } catch (_) { /* 計測で本体を壊さない */ }
  ...
  async wrap(name, fn, details) {
    ...
    try {
      ...
      this.#safeSpan(name, startedAt, details === undefined ? {} : { details });
    }
  }

  /** 観測の失敗で本処理 (書き込み・送信) を壊さないための隔離。 */
  #safeSpan(name, startedAt, extra) {
    try {
      return this.span(name, startedAt, extra);
    } catch {
      return null;
    }
  }
  ...
  begin(name, details) {
    const startedAt = this.#now();
    return {
      end: (extra) => this.#safeSpan(name, startedAt, {
