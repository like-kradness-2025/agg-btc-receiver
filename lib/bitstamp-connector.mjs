...
export class BitstampConnector extends TradeOnlyConnector {
  ...
  _onMessage(data) {
    ...
    if (event === 'bts:subscription_succeeded' || event === 'bts:unsubscription_succeeded' || event === 'bts:heartbeat') {
      return;
    }
    if (event === 'bts:request_reconnect') {
      // 公式 (WebSocket API v2): サーバーからの再接続要求。無視すると切断まで
      // 無音のまま取り残されるため、自分から張り直す (2026-09-23)。
      this._proactiveReconnect('bitstamp bts:request_reconnect');
      return;
    }
