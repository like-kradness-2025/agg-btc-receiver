  ...
  async _syncBook() {
    ...
    for (let attempt = 0; attempt < MAX_SYNC_ATTEMPTS; attempt++) {
      try {
        ...
        // snapshot and a newer diff can never be lost.
        const pending = this._pendingDepth;
        const replay = [];
        let boundaryUnprovable = false;
        for (const event of pending) {
          if (event.ts == null) {
            // Unparseable source time: the diff can never be applied by this
            // connector (same fail-closed drop as steady state), and it can
            // not be used to locate the boundary — drop it, counted.
            this._stats.droppedDepthCount++;
            continue;
          }
          if (event.ts < boundaryMs) {
            ...
            this._stats.snapshotIncludedDiffCount++;
            continue;
          }
          if (event.ts === boundaryMs) {
        ...
        if (boundaryUnprovable) {
          ...
          throw new Error(`buffered diff at ts=${pending.find((e) => e.ts != null && e.ts === boundaryMs)?.ts} equals snapshot boundary; re-sync with newer snapshot (fail-closed)`);
        }
        ...
        if (replay.length) {
          this._lastAppliedDiffTsMs = replay[replay.length - 1].ts;
        } else {
          this._lastAppliedDiffTsMs = boundaryMs;
        }
        this._pendingDepth = [];
