...
import { BaseConnector, CANONICAL_UNPARSED_STREAM } from './base-connector.mjs';
...
export class BinanceSpotConnector extends BaseConnector {
  ...
  _onMessage(data) {
    ...
    else if (event.e === 'serverShutdown') {
      // 公式: サーバー停止前に届く。「できるだけ早く新規接続を張れ」(General WSS information)。
      this._proactiveReconnect('binance serverShutdown');
    }
  }

  /**
   * Issue #10 canonical capture: Binance combined-stream frames are
   * self-describing ({ stream: 'symbol@channel', data: {...} }). Classify the
   * stream token only — the frame payload itself is stored verbatim and never
   * interpreted by the canonical path.
   */
  _canonicalFrameMeta(data) {
    const streamName = data?.stream;
    if (typeof streamName !== 'string') return null;
    if (streamName.includes('@trade') || streamName.includes('@aggTrade')) {
      return { stream: 'trades', channel: streamName };
    }
    if (streamName.includes('@depth')) return { stream: 'book_updates', channel: streamName };
    if (streamName.includes('@forceOrder')) return { stream: 'liquidations', channel: streamName };
    return null;
  }

  /**
   * Issue #10 pre-parse capture: combined-stream frames that fail JSON.parse
   * cannot be content-classified; preserve the exact failing text under the
   * reserved parse_failed classification so the pre-parser log has no holes.
   */
  _canonicalUnparsedFrameMeta() {
    return { stream: CANONICAL_UNPARSED_STREAM, channel: null };
  }
  ...
  _handleTrade(event) {
    // R-11: never drop a trade silently. `_emitTrade` is the single fail-closed
    // guard that validates price/qty/ts and counts every rejection in
    // `droppedTradeCount`; an early `return` here (the previous `if (!price ||
    // !qty) return;`) skipped that counter, so a zero/NaN price or qty — e.g. a
    // malformed or forceOrder-derived frame — was lost with no trace in
    // health.jsonl.
    this._emitTrade(
      parseFloat(event.p),
      parseFloat(event.q),
  ...
  _applyRingBuf(snapshot) {
    ...
    for (const msg of replay) {
      ...
      }, msg._ingress ?? null)) throw new Error('Binance spot buffered depth emission rejected');
      this._applyDiff(msg);
...
export class BinancePerpConnector extends BaseConnector {
  ...
  _onMessage(data) {
    ...
    if (event.e === 'depthUpdate') this._handleDepth(event);
      ...
      // 公式: サーバー停止前に届く。「できるだけ早く新規接続を張れ」。
      this._proactiveReconnect('binance serverShutdown');
    }
  }

  /**
   * Issue #10 canonical capture: same combined-stream classifier as spot
   * (adds aggTrade / forceOrder mapping).
   */
  _canonicalFrameMeta(data) {
    const streamName = data?.stream;
    if (typeof streamName !== 'string') return null;
  ...
  _handleTrade(event) {
    // R-11: see the spot override above — always route through _emitTrade so a
    // zero/NaN price or qty is counted in droppedTradeCount instead of being
  ...
  _handleForceOrder(event) {
    ...
    this._emitLiquidation({
      ...
      raw_type: 'forceOrder',
      trade_id: null,
      // Issue #12: never substitute receive time for a missing exchange time.
      source_ts: o.T ?? event.E ?? null,
    });
  }
  ...
  _applyRingBuf(snapshot) {
    ...
    for (const msg of replay) {
      if (!this._emitDepth('update', msg.b.map(([p, q]) => [p, q]), msg.a.map(([p, q]) => [p, q]), msg.E, msg.u, {
        seq_start: msg.U, seq_end: msg.u, prev_seq: lastAppliedSeq,
      }, msg._ingress ?? null)) throw new Error('Binance perp buffered depth emission rejected');
      this._applyDiff(msg);
      this.book.setLastSeq(msg.u);
      lastAppliedSeq = msg.u;
    }
