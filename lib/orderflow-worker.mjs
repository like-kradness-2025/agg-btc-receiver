...
import { flushEnvelopeQueue } from './ipc-flush.mjs';
...
function flushRawIpc() {
  // 観測失敗で送信経路を壊さない (lib/ipc-flush.mjs を参照)。
  flushEnvelopeQueue({
    queue: rawIpcPending, port: parentPort, messageType: 'rawEvents',
    spanName: 'worker.ipc_send_raw', probe: stallProbe,
  });
}
...
function flushCanonicalIpc() {
  flushEnvelopeQueue({
    queue: canonicalIpcPending, port: parentPort, messageType: 'canonicalFrames',
    spanName: 'worker.ipc_send_canonical', probe: stallProbe,
  });
}
