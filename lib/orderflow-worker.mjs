...
import { StallProbe } from './stall-probe.mjs';
...
import { MarketStatusTracker } from './market-status.mjs';
...
// Stall observer (2026-09-21): ワーカースレッド側の停止を検出して main に渡す。
// 記録は異常時のみ (main が単一ライターとして stall-events.jsonl に書く)。
const stallProbe = new StallProbe({
  label: 'worker',
  lagThresholdMs: Number(process.env.RECEIVER_STALL_LAG_MS) || 1500,
  sampleMs: 250,
  minSpanMs: 25, // ソケットメッセージ毎の計測はここで捨てる (高頻度パスのため)
  onAnomaly: (record) => {
    try {
      parentPort.postMessage({ type: 'stallAnomaly', record: { ...record, label: `worker:${workerId}` } });
    } catch { /* 監視の失敗で本体を落とさない */ }
  },
});
stallProbe.start();
...
// Issue #16: per-market runtime state + data_complete tracking. `ready` says
// the process started and every connect attempt finished; data_complete says
// every required market in THIS worker is currently streaming with a synced
// book. A degraded market keeps the worker ready but completeness false.
let marketStatus = null;
let lastWorkerDataComplete = null;
...
// ── Issue #12 timestamp helpers ──────────────────────────────────────────
// The worker never invents a receive time: enqueue time is not receive time.
// Missing/invalid timestamps stay null (unknown) and only the writer's own
// write-time columns (written_at_ms / batch recv fallback) observe enqueue.

// Astra P2-3: strict type-check BEFORE numeric coercion. Number(null)===0,
// Number('')===0 and Number(false)===0 would otherwise turn missing values
// into a "known" ts=0 (and Number('-1') into a known -1) — a missing
// timestamp must stay null (unknown). Only number / numeric-string inputs
// are accepted; negatives are never valid wall/monotonic clocks.

/** Finite number → itself; anything else (incl. ''/false/-1) → null. */
...
/** Finite number → truncated integer ms; anything else (incl. ''/false/-1) → null. */
const finiteMsOrNull = (value) => {
  if (value === null || value === undefined) return null;
  if (typeof value !== 'number' && typeof value !== 'string') return null;
  if (typeof value === 'string' && value.trim() === '') return null;
  const n = Number(value);
...
let rawIngestSeq = 0;
// Canonical raw frames (issues #10/#11) travel worker → main on their own
// IPC channel so the main process routes them to the append-only writer and
// never mixes them into the legacy mutable raw_batches path.
const canonicalIpcPending = [];
let canonicalIpcFlushTimer = null;
...
function drainRawWriteState(state) {
  ...
  state.drainPromise = (async () => {
    while (state.pending.length) {
      ...
      try {
        await stallProbe.wrap('worker.writeBatch', () => state.writer.writeBatch(batch), { rows: batch.length });
      } catch (error) {
...
/**
 * Build the raw-DB envelope (raw_v5_duckdb / raw_v6_sqlite) for one emitted
 * event. Exported for unit tests; the worker never imports this module in the
 * main thread otherwise.
 *
 * Issue #12 contract:
 *   - recv_ts_ms / recv_mono_ns / receive_seq / connection_id are the
 *     connector-stamped ingress metadata and pass through UNCHANGED — the
 *     worker never re-stamps receive time at enqueue.
 *   - a missing/invalid recv_ts_ms stays null (unknown): the enqueue moment
 *     is not a receive time, so no Date.now() substitution happens here.
 *   - source_event_ts_ms / source_event_time_known pass through; legacy
 *     events without them default the source time to the event ts when valid.
 *   - payload carries the original event verbatim (non-enumerable _ingress
 *     snapshots never leak into it) plus worker metadata.
 *
 * @param {Object} args
 * @param {Object} args.obj - emitted event (trade/depth/liquidation/...)
 * @param {number|null} args.eventTimestampMs - caller-provided event ts fallback
 * @param {'duckdb'|'sqlite'} args.storage
 * @param {string} args.market
 * @param {string} args.kind - stream kind ('trades'|'book_updates'|...)
 * @param {string} args.workerId
 * @param {number} args.workerSeq
 * @returns {Object} envelope
 */
export function buildRawDbEnvelope({ obj, eventTimestampMs, storage, market, kind, workerId, workerSeq }) {
  const eventTs = finiteMsOrNull(obj?.event_ts_ms ?? obj?.ts ?? eventTimestampMs);
  const sourceTs = obj?.source_event_ts_ms !== undefined
    ? finiteMsOrNull(obj.source_event_ts_ms)
    : eventTs;
  const rawMetadata = {
  ...
  return {
    schema: storage === 'sqlite' ? 'raw_v6_sqlite' : 'raw_v5_duckdb',
    market,
    stream: kind,
    event_ts_ms: eventTs,
    recv_ts_ms: finiteMsOrNull(obj?.recv_ts_ms),
    recv_mono_ns: finiteOrNull(obj?.recv_mono_ns),
    receive_seq: obj?.receive_seq ?? null,
    source_event_ts_ms: sourceTs,
    source_event_time_known: obj?.source_event_time_known !== undefined
      ? !!obj.source_event_time_known
      : sourceTs !== null,
...
function queueRawWrite(writer, obj, eventTimestampMs) {
  ...
  if (writer.storage === 'duckdb' || writer.storage === 'sqlite') {
    ...
    // Pre-built v4 raw frames pass through untouched (defensive legacy path).
    if (obj?.schema === 'raw_v4') {
      rawIpcPending.push(obj);
      if (rawIpcPending.length >= RAW_IPC_BATCH_SIZE) flushRawIpc();
      else scheduleRawIpcFlush();
      return;
    }
    rawIpcPending.push(buildRawDbEnvelope({
      obj,
      eventTimestampMs,
...
const stallGaugeTimer = setInterval(() => {
  stallProbe.note('rawIpcPending', rawIpcPending.length);
  stallProbe.note('canonicalIpcPending', canonicalIpcPending.length);
}, 1000);
stallGaugeTimer.unref?.();

export function frameBytesOf(envelopes) {
  let bytes = 0;
...
function flushRawIpc() {
  ...
  // postMessage の構造化クローンは送信側スレッドで同期実行されるため、停止の
  // 主因になりうる (Astra P1⑦)。自処理(writeBatch)と区別できるよう span にする。
  const end = stallProbe.begin('worker.ipc_send_raw', {
    envelopes: envelopes.length, frame_bytes: frameBytesOf(envelopes),
  });
  try {
    parentPort.postMessage({ type: 'rawEvents', envelopes });
  } finally {
    // begin() が関数を返さない場合に送信経路ごと落とさない (本番で起動失敗ループを起こした)
...
// Canonical raw (issues #10/#11): queue one immutable source frame captured at
// the connector socket boundary. Envelopes keep connector arrival order.
// Astra P2: only MISSING / non-string frame_text is rejected. An empty string
// ('') is a legitimate byte-exact source frame (e.g. an empty socket message
// whose JSON.parse failed at the boundary) and must not be dropped pre-IPC.
function queueCanonicalFrame(frame) {
  if (!frame || typeof frame.frame_text !== 'string') return;
  canonicalIpcPending.push(frame);
  if (canonicalIpcPending.length >= RAW_IPC_BATCH_SIZE) flushCanonicalIpc();
  else scheduleCanonicalIpcFlush();
}

function flushCanonicalIpc() {
  if (!canonicalIpcPending.length) return;
  const envelopes = canonicalIpcPending.splice(0, canonicalIpcPending.length);
  const end = stallProbe.begin('worker.ipc_send_canonical', {
    envelopes: envelopes.length, frame_bytes: frameBytesOf(envelopes),
  });
  try {
    parentPort.postMessage({ type: 'canonicalFrames', envelopes });
  } finally {
    // begin() が関数を返さない場合に送信経路ごと落とさない (本番で起動失敗ループを起こした)
    if (typeof end === 'function') end();
  }
...
function scheduleCanonicalIpcFlush() {
  if (canonicalIpcFlushTimer) return;
  canonicalIpcFlushTimer = setTimeout(() => {
    canonicalIpcFlushTimer = null;
    flushCanonicalIpc();
  }, RAW_IPC_FLUSH_MS);
  canonicalIpcFlushTimer.unref?.();
}
...
/**
 * Issue #16: broadcast this worker's per-market status + aggregate
 * data_complete to the main thread and log completeness transitions. Called
 * after every connector state change / degradation / recovery so the main
 * thread never conflates "worker ready" with "data complete".
 */
function broadcastMarketStatus(market, { state, degradedReason } = {}) {
  if (!marketStatus) return;
  const snap = marketStatus.snapshot();
  if (lastWorkerDataComplete !== snap.data_complete) {
    // data_complete=false is an ERROR-class condition for downstream (docs:
    // 'must not present as a normal aggregate') — log it on stderr so the
    // receiver's log routing treats it like the main thread's report.
    const line =
      `[worker:${workerId}] data_complete=${snap.data_complete} ` +
      `(running=${snap.running_markets.length}/${snap.expected_markets.length}, ` +
      `degraded=${Object.keys(snap.degraded_markets).length}: ${Object.keys(snap.degraded_markets).join(',') || '-'})`;
    if (snap.data_complete) console.log(line);
    else console.error(line);
    lastWorkerDataComplete = snap.data_complete;
  }
  const entry = market ? snap.markets[market] : null;
  parentPort.postMessage({
    type: 'marketStatus',
    workerId,
    market,
    state: state ?? entry?.state ?? null,
    degradedReason: degradedReason ?? entry?.degraded_reason ?? null,
    dataComplete: snap.data_complete,
    runningMarkets: snap.running_markets,
    degradedMarkets: Object.keys(snap.degraded_markets),
    expectedMarkets: snap.expected_markets,
  });
}

/** Issue #16: worker report carried inside the 'ready' IPC message. */
function workerCompletenessReport() {
  const snap = marketStatus?.snapshot?.();
  // Fail-visible default: if the tracker was never initialised there is no
  // evidence any required market streams — report incomplete, never assume
  // complete (optimistic defaults hide outages).
  if (!snap) return { dataComplete: false, markets: [] };
  return {
    dataComplete: snap.data_complete,
    markets: Object.entries(snap.markets).map(([m, s]) => ({
      market: m,
      state: s.state,
      degradedReason: s.degraded_reason,
    })),
  };
}
...
async function prepareMarket(market) {
  ...
  // Issue #10/#11: canonical raw frames captured at the socket boundary are
  // forwarded on their own IPC channel (append-only sink in the main
  // process). Active only on the SQLite storage path where the canonical
  // table lives; the legacy DuckDB path is untouched.
  if (rawDbStorage() && configOutput.raw_storage === 'sqlite') {
    connector.on('canonicalFrame', (frame) => queueCanonicalFrame(frame));
  }
  ...
  connector.on('stateChange', (from, to) => {
    ...
    marketStatus?.observeState(market, to);
    broadcastMarketStatus(market);
    parentPort.postMessage({
      type: 'stateChange',
      market,
      from,
...
function scheduleInitialReconnect(market, connector, reason) {
  ...
  // Issue #16: degraded isolation keeps the worker alive and ready, but the
  // market is NOT running — data_complete must reflect that immediately.
  marketStatus?.markDegraded(market, reason);
  broadcastMarketStatus(market);
}
...
async function restartMarket(market, reason = 'operator request') {
  ...
  const promise = (async () => {
    ...
    try {
      ...
      // PR #20 re-audit: this worker's own tracker must mirror the main thread —
      // a failed module restart means the market is NOT streaming (its
      // connector was disconnected and the reseed failed). The watchdog
      // restarts markets whose connector still ticks a stale 'running', so
      // without this the worker-side completeness would keep reporting true
      // until the background reconnect happens to emit a stateChange.
      marketStatus?.markDegraded(market, `module restart failed: ${error.message}`);
      broadcastMarketStatus(market);
      connector._scheduleReconnect?.();
      throw error;
    }
...
async function doInit(msg) {
  ...
  // Issue #16: per-market runtime state for this worker's own completeness.
  marketStatus = new MarketStatusTracker({
    expectedMarkets: markets,
    optionalMarkets: [...optionalMarkets],
  });
  lastWorkerDataComplete = null;
  ...
  // Signal ready. Issue #16: ready = process up + every connect attempt done.
  // It deliberately does NOT imply data completeness — degraded markets are
  // reported alongside so the main thread can separate the two concepts.
  parentPort.postMessage({ type: 'ready', workerId, ...workerCompletenessReport() });
}
...
async function doShutdown(exitCode = 0) {
  ...
  if (canonicalIpcFlushTimer) { clearTimeout(canonicalIpcFlushTimer); canonicalIpcFlushTimer = null; }
...
// parentPort is null when this module is imported from the main thread
// (unit tests import buildRawDbEnvelope directly). Registering the handler
// is only meaningful inside a worker thread.
if (parentPort) {
