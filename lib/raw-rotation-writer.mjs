...
export class RawRotationWriter {
  ...
  constructor(basePath, market, kind, options = {}) {
    ...
    // I/O error tracking (observable via getIoFailure)
    /** @type {number} */
    this._ioFailureCount = 0;
    /** @type {string|null} */
    this._lastIoErrorMessage = null;

    // Late-event visibility (observable via getLateEventStats)
    /** @type {number} — total events routed to .jsonl.late sidecars */
    this._lateEventCount = 0;
    /** @type {{windowMs: number, reason: string, atMs: number}|null} */
    this._lastLateEventInfo = null;
  }
  ...
  /**
   * Write a batch in one serialized queue task.
   *
   * The receiver can produce thousands of book updates per second.  Queueing
   * one Promise-chain task per event lets the queue grow faster than the
   * filesystem can drain it.  Batch callers keep the same ordering and error
   * semantics while paying the queue/microtask overhead once per batch.
   *
   * @param {Array<[object, number]>} events
   * @returns {Promise<void>}
   */
  async writeBatch(events) {
    ...
    try {
      await this._enqueue(async () => {
        for (const [obj, eventTimestampMs] of events) {
          try {
            await this._writeImpl(obj, eventTimestampMs);
          } catch (err) {
            this._recordIoError(err);
            console.error(
              `[RawRotationWriter] ${this._market}/${this._kind}: write error:`,
      ...
      this._recordIoError(err);
      console.error(
        `[RawRotationWriter] ${this._market}/${this._kind}: batch write error:`,
        err.message,
      );
    }
  }
  ...
  /**
   * Flush buffered lines for the current and previous windows without
   * finalizing either file.
   *
   * Rotation is intentionally separate from flushing: the active .open file
   * must become durable during a long-lived window, while remaining writable
   * for late events until the normal watermark/finalize rules apply.
   *
   * @returns {Promise<void>}
   */
  async flush() {
    try {
      await this._enqueue(() => this._flushImpl());
    } catch (err) {
      this._recordIoError(err);
      console.error(
        `[RawRotationWriter] ${this._market}/${this._kind}: flush error:`,
        err.message,
      );
    }
  }
  ...
  /**
   * Get accumulated I/O failure info.
   *
   * Returns { count, message } where count is the number of I/O errors
   * recorded since construction, and message is the last error message
   * (or null if no errors).  Callers use this to detect silent persistence
   * failures and report them via health/IPC.
   *
   * @returns {{ count: number, message: string|null }}
   */
  getIoFailure() {
    return {
  ...
  /**
   * Record an I/O error internally so it is observable via getIoFailure().
   *
   * @param {Error} err
   */
  _recordIoError(err) {
    this._ioFailureCount++;
    this._lastIoErrorMessage = err.message;
  }
  ...
  async _writeImpl(obj, eventTimestampMs) {
    ...
    if (
      ...
      // Late event behind the watermark — do NOT drop.  The window file is
      // already finalized (immutable contract for readers), so record the
      // event in a per-window sidecar (.jsonl.late) that downstream merge
      // tooling can fold back into the timeline.
      await this._writeLateEvent(obj, tsMs, wMs, 'behind_watermark');
      return;
    }
  ...
  /** Flush active writers without changing their rotation state. */
  async _flushImpl() {
    if (this._previousWriter) await this._previousWriter.flush();
    if (this._currentWriter) await this._currentWriter.flush();
    // R-05: BufferedWriter records flush failures internally instead of
    // throwing on the fire-and-forget paths, so surface them here. Without
    // this the health path (getIoFailure -> HealthMonitor) reported count:0
    // while buffered raw lines were being dropped.
    this._drainWriterIoFailure(this._previousWriter);
    this._drainWriterIoFailure(this._currentWriter);
  }

  /**
   * R-05: propagate a BufferedWriter's recorded I/O failures into this
   * writer's own getIoFailure() counter (health-observable, deduplicated so
   * one underlying failure is counted once per writer instance).
   *
   * @param {BufferedWriter|null} writer
   */
  _drainWriterIoFailure(writer) {
    if (!writer) return;
    const count = Number(writer._ioFailureCount ?? 0);
    const seen = Number(writer._ioFailureSeenCount ?? 0);
    if (!(count > seen)) return;
    writer._ioFailureSeenCount = count;
    this._recordIoError(new Error(
      `buffered writer I/O failure (${path.basename(writer._filePath)}): `
      + `${writer._lastIoErrorMessage ?? 'unknown'}`,
    ));
    console.error(
      `[RawRotationWriter] ${this._market}/${this._kind}: ${writer._ioFailureCount} buffered I/O failure(s) `
      + `on ${path.basename(writer._filePath)}: ${writer._lastIoErrorMessage}`,
    );
  }
  ...
  // ---- Internal: late-event sidecar ---------------------------------------

  /**
   * Record a too-late event in the per-window sidecar instead of dropping it.
   *
   * Sidecar path: <window>.jsonl.late — one JSON object per line:
   *   {"ts":<event ts ms>,"recv_ts":<write time ms>,"reason":"...","obj":<event>}
   *
   * Appends are serialized by this writer's queue, so no extra locking is
   * needed.  I/O failures are recorded but never thrown (same semantics as
   * normal writes).  The sidecar intentionally lives outside the watermark
   * lifecycle: finalize/checkStale never touch .jsonl.late files.
   *
   * @param {object} obj
   * @param {number} tsMs - event timestamp ms
   * @param {number} wMs - window start ms for the event
   * @param {string} reason
   */
  async _writeLateEvent(obj, tsMs, wMs, reason) {
    this._lateEventCount += 1;
    this._lastLateEventInfo = { windowMs: wMs, reason, atMs: Date.now() };

    const { dateDir, fileBase } = windowStartToDateStr(wMs);
    const dir = path.join(this._basePath, this._kind, this._market, dateDir);
    const sidecarPath = path.join(dir, `${fileBase}.jsonl.late`);
    ...
    try {
      ...
      await fsp.appendFile(sidecarPath, line, 'utf8');
      console.error(
        `[RawRotationWriter] ${this._market}/${this._kind}: late event ` +
        `(ts=${tsMs}, window=${wMs} <= watermark ${this._finalizedWatermarkMs}) ` +
        `recorded to sidecar`,
      );
    } catch (err) {
      this._recordIoError(err);
      console.error(
        `[RawRotationWriter] ${this._market}/${this._kind}: failed to write late sidecar:`,
        err.message,
      );
    }
  }

  /** @returns {{count: number, last: {windowMs: number, reason: string, atMs: number}|null}} */
  getLateEventStats() {
    return { count: this._lateEventCount, last: this._lastLateEventInfo };
  }
  ...
  async _finalizeImpl() {
    if (this._previousWriter) {
      try {
        ...
        // R-05: keep the writer (and its unflushed buffer) reachable so a
        // later flush/finalize can still persist it once I/O recovers.
        this._recordIoError(err);
        console.error(
          `[RawRotationWriter] ${this._market}/${this._kind}: finalize error (previous writer, window ${this._previousWindowMs}):`,
          err.message,
        );
      }
    }
    if (this._currentWriter) {
      try {
        ...
        this._recordIoError(err);
        console.error(
          `[RawRotationWriter] ${this._market}/${this._kind}: finalize error (current writer, window ${this._currentWindowMs}):`,
          err.message,
        );
      }
    }
  ...
  async _startupRecoveryImpl(nowMs) {
    ...
    const reopenableWindows = new Set([
      nowWindowMs,
      nowWindowMs - WINDOW_MS,
    ]);

    // A graceful receiver restart must not make the just-closed current
    // window immutable. Events arriving after reconnect can still belong to
    // that window. Reopen finalized active windows so the new process can
    // append them instead of dropping them behind the watermark.
    for (const finalPath of jsonlFiles) {
      const wMs = this._extractWindowMs(finalPath);
      if (wMs === null || !reopenableWindows.has(wMs)) continue;
      const openPath = `${finalPath}.open`;
      try {
        await fsp.access(openPath);
        const [finalData, openData] = await Promise.all([
          fsp.readFile(finalPath),
          fsp.readFile(openPath),
        ]);
        const mergedPath = `${openPath}.recovery-${process.pid}-${wMs}`;
        await fsp.writeFile(mergedPath, Buffer.concat([finalData, openData]), { flag: 'wx' });
        await fsp.rename(mergedPath, openPath);
        await fsp.unlink(finalPath);
      } catch (err) {
        if (err.code === 'ENOENT') {
          await fsp.rename(finalPath, openPath);
        } else {
          throw err;
        }
      }
    ...
    for (const fp of jsonlFiles) {
      ...
      if (w !== null) {
        if (reopenableWindows.has(w)) continue;
        if (
          this._finalizedWatermarkMs === null ||
...
// ---------------------------------------------------------------------------
// 5. Output-root lock (re-exported from ./lock.mjs)
// ---------------------------------------------------------------------------
//
// The canonical output-root lock implementation lives in lib/lock.mjs
// (flock-based, kernel-backed, no stale-recovery races).
// We re-export so callers importing from this module still get the lock.

export { acquireOutputRootLock, releaseOutputRootLock } from './lock.mjs';
