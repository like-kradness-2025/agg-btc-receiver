...
export class CoinbaseConnector extends BaseConnector {
  ...
  _l2Continuity(frameSeq, localSeq) {
    ...
    // Known limitation (Astra audit #19 P2-4): localSeq == null → the frame
    // is unconditionally anchored ('ok') because there is no applied seq to
    // prove continuity against. Real Coinbase l2_data snapshots always carry
    // sequence_num, so the WS-snapshot path always leaves an anchor; this
    // branch is only reachable after the REST-fallback snapshot (seq is
    // explicitly null there — REST and WS sequence domains differ, so the
    // REST value must never anchor the WS stream) or in book states with no
    // applied frame yet. Accepting the first sequenced WS frame as the new
    // anchor is the only safe choice in that state; it cannot be verified
    // against the REST domain, so it deliberately acts like the snapshot→
    // first-update bridge.
