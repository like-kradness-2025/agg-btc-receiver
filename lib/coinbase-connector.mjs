...
// assert the contract; keep in sync with L2_SEQ_SKIP_TOLERANCE. Per-instance
// override via config.l2SeqSkipTolerance for re-calibration without redeploy.
...
export class CoinbaseConnector extends BaseConnector {
  constructor(config) {
    ...
    // Issue #22 P1 (audit): observability of within-tolerance coalesced skips
    // — count of ok verdicts with delta > 1 and the largest skip delta seen
    // (any verdict). Lets ops verify TOL stays above the live coalescing
    // ceiling instead of discovering drift only via the next gap incident.
    this._stats.l2TolSkipCount = 0;
    this._stats.l2MaxSeqSkipDelta = 0;
    // Config override is outside the fail-closed trust boundary: sanitize so a
    // NaN/0/negative/huge value can never disable gap detection (NaN compares
    // false → permanent fail-open; 0/negative → every forward frame gaps,
    // recreating the resync loop this fix removes) or silently widen it.
    const tolOverride = Number(config.l2SeqSkipTolerance);
    this._l2SeqSkipTolerance = Number.isFinite(tolOverride) && tolOverride >= 1 ? Math.floor(tolOverride) : L2_SEQ_SKIP_TOLERANCE;
  }
  ...
  _l2Continuity(frameSeq, localSeq) {
    ...
    const skipDelta = frameSeq - localSeq;
    if (skipDelta > this._l2SeqSkipTolerance) {
      // Track the largest observed skip regardless of verdict (observability:
      // shows how close live coalescing gets to the fail-closed boundary).
      if (skipDelta > this._stats.l2MaxSeqSkipDelta) this._stats.l2MaxSeqSkipDelta = skipDelta;
      return 'gap';
    }
    ...
    // Only a skip beyond the tolerance is treated as a real drop.
    if (skipDelta > 1) {
      this._stats.l2TolSkipCount++;
      if (skipDelta > this._stats.l2MaxSeqSkipDelta) this._stats.l2MaxSeqSkipDelta = skipDelta;
    }
    return 'ok';
  }
