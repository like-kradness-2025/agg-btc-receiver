...
export function computeFeatures1s({ detector, blockStartMs, tradeTsList, warmup, inputBlockIds, lookupTradedNotional30s, bookSnapshot }) {
  ...
  for (const secondTs of generateSeconds(blockStartMs)) {
    ...
    if (bookSnapshot?.available && bookSnapshot?.state?.seeded && bookSnapshot.state.best_bid != null && bookSnapshot.state.best_ask != null) {
      ...
      // ── Book features B1-B9 (compute when book seeded) ──
      ...
      // Note: bookSnapshot.state only has best levels, need full book for depth
      // For now, use best level as proxy (depth at best level only)
      // TODO: When full book state available, compute actual depth windows
      row.book_bid_depth_100 = best_bid * best_bid_qty;
      row.book_ask_depth_100 = best_ask * best_ask_qty;
      row.book_bid_depth_1000 = best_bid * best_bid_qty;
      row.book_ask_depth_1000 = best_ask * best_ask_qty;
      ...
      row.book_imbalance_100 = totalDepth100 > 0
        ? (row.book_bid_depth_100 - row.book_ask_depth_100) / totalDepth100
        : null;
      ...
      row.book_imbalance_1000 = totalDepth1000 > 0
        ? (row.book_bid_depth_1000 - row.book_ask_depth_1000) / totalDepth1000
        : null;

      // B9: book_microprice
      const totalQty = best_bid_qty + best_ask_qty;
      row.book_microprice = totalQty > 0
        ? (best_ask * best_bid_qty + best_bid * best_ask_qty) / totalQty
        : null;
    }
