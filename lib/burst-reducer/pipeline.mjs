  ...
  for (const root of roots) {
    ...
    for (const date of readdirSync(root)) {
      ...
      for (const name of names) {
        ...
        let raw;
        try { raw = JSON.parse(readFileSync(path, 'utf8').split('\\n').find(Boolean)); } catch { continue; }
        if (raw?.market !== market || typeof raw.ts !== 'number' || raw.ts > blockStartMs) continue;
        if (!best || raw.ts > best.raw.ts) best = { path, raw };
      }
    }
  }
  return best;
