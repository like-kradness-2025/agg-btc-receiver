...
import { readFileSync, existsSync, mkdirSync, writeFileSync, rmSync, renameSync, readdirSync } from 'node:fs';
...
import { buildRawTradedNotionalLookup, buildRawTradeHistory } from './raw-trades-notional-reader.mjs';
import { validateRawTradeLookback } from './raw-trades-notional-reader.mjs';
import { buildV4RawTradedNotionalLookup, buildV4RawTradeHistory, validateV4RawTradeLookback } from './raw-trades-notional-reader.mjs';
import { RawV4BlockSource } from '../downstream/raw-v4-block-source.mjs';
...
function isV4Block(block) {
  return block && typeof block.fullPath === 'string' && block.fullPath.startsWith('__raw_v4__:');
}

function readBlockContent(block) {
  if (isV4Block(block)) return block.content;
  return readFileSync(block.fullPath, 'utf8');
}

function sha256Block(block) {
  if (isV4Block(block)) return block.sha256;
  return sha256File(block.fullPath);
}

function readPendingBlockContent(pendingBlock, rawLayout, v4Source) {
  if (rawLayout === 'v4') {
    const block = v4Source.getBlock(pendingBlock.block_start_ms);
    if (!block) throw new Error(`v4 block not found for pending ${pendingBlock.block_start_ms}`);
    return block.content;
  }
  return readFileSync(pendingBlock.replay_identity.input_path, 'utf8');
}

function getV4SegmentProof(v4Source, blockStartMs) {
  return v4Source?.getBlock(blockStartMs)?.segmentProof || null;
}
 ...
 */
function checkFinalizedHorizon(pendingBlock, finalizedThroughMs, frozenInventory, dataDir, market, kind = 'trades', pendingIsActive = false) {
  if (!pendingBlock) return { canFinalize: false, state: 'no-pending', reason: 'no-pending-block', horizonProof: null };

  // v4: active incomplete trailing block must not be finalized
  if (pendingIsActive) {
    return { canFinalize: false, state: 'active-incomplete', reason: 'active-incomplete-block', horizonProof: null };
  }
...
const seedSnapshotIndexCache = new Map();

function getSeedSnapshotIndex(dataDir, market) {
  const cacheKey = `${dataDir}:${market}`;
  if (seedSnapshotIndexCache.has(cacheKey)) return seedSnapshotIndexCache.get(cacheKey);
  const entries = [];
  const roots = [join(dataDir, 'snapshots', market), join(dataDir, 'book_snapshots', market)];
  for (const root of roots) {
    if (!existsSync(root)) continue;
    for (const date of readdirSync(root)) {
      const dateDir = join(root, date);
      let names;
      try { names = readdirSync(dateDir).filter((name) => name.endsWith('.jsonl')); } catch { continue; }
      for (const name of names) {
        const path = join(dateDir, name);
        try {
          const raw = JSON.parse(readFileSync(path, 'utf8').split('\n').find(Boolean));
          if (raw?.market === market && typeof raw.ts === 'number') entries.push({ path, raw });
        } catch { /* corrupt snapshot is skipped; book remains fail-closed */ }
      }
    }
  }
  entries.sort((a, b) => a.raw.ts - b.raw.ts);
  seedSnapshotIndexCache.set(cacheKey, entries);
  return entries;
}

function findLatestSeedSnapshot(dataDir, market, blockStartMs) {
  const entries = getSeedSnapshotIndex(dataDir, market);
  let lo = 0;
  let hi = entries.length;
  while (lo < hi) {
    const mid = (lo + hi) >>> 1;
    if (entries[mid].raw.ts <= blockStartMs) lo = mid + 1;
    else hi = mid;
  }
...
/**
 * Load book snapshot plus same-block updates. The raw receiver stores seed
 * snapshots under snapshots/ or book_snapshots/, while incremental events
 * live under book_updates/. The seed is inserted as a canonical snapshot
 * before updates so the replay can produce seeded best bid/ask state.
 */
function loadBookSnapshot(dataDir, market, blockStartMs) {
  ...
  const events = [];
  const seed = findLatestSeedSnapshot(dataDir, market, blockStartMs);
  if (seed) {
    const seedResult = toCanonicalBookEnvelope({ ...seed.raw, type: 'snapshot' });
    if (!seedResult.valid) return { available: false, book_seeded: false };
  ...
  const stateResult = stateAt(events, blockStartMs + 30000, { includeLevels: true });

  // Build strict pre-second states so each 1s row is as-of and cannot see
  // book updates from the current/future second.
  const statesBySecond = new Map();
  for (let secondTs = blockStartMs; secondTs < blockStartMs + BLOCK_DURATION_MS; secondTs += 1000) {
    const secondState = stateAt(events, secondTs, { includeLevels: true });
    if (secondState && !secondState.quarantined && secondState.seeded) {
      statesBySecond.set(secondTs, secondState);
    }
  }

  // Raw update stream may begin after a sequence gap from the seed snapshot.
  // Keep the strict quarantine result, but allow a clearly marked snapshot-only
  // observation for diagnostics/feature conversion; no update is treated as valid.
  if (stateResult && stateResult.quarantined && seed) {
    const seedOnly = stateAt([events[0]], blockStartMs + 30000, { includeLevels: true });
    if (seedOnly && !seedOnly.quarantined && seedOnly.seeded) {
    return { available: true, book_seeded: true, book_source: 'snapshot_fallback', state: seedOnly, statesBySecond, events, stateAt: (anchor) => stateAt(events, anchor, { includeLevels: true }) };
    }
  }
  ...
  return { available: true, book_seeded: bookSeeded, state: stateResult || null, statesBySecond, events, stateAt: (anchor) => stateAt(events, anchor, { includeLevels: true }) };
}
 ...
 * @param {string} [params.rawLayout='v3'] - 'v3' (30s files) or 'v4' (hourly append segments)
 */
export async function runPipeline({ dataDir, market, fromMs, toMs, runId, outputRoot, finalizedThroughMs = null, frozenInventory = null, kind = 'trades', rawLayout = 'v3' }) {
  ...
  if (rawLayout === 'v4') {
    v4Cursor = cp?.pending_block?.replay_identity?.v4_cursor ?? cp?.raw_v4_cursor ?? null;
    v4Source = new RawV4BlockSource({ root: dataDir, kind, market, cursor: v4Cursor });
    await v4Source.open();
  }

  try {
    if (rawLayout === 'v4') {
      const loaded = await v4Source.loadBlocks({ fromMs: effectiveFromMs, toMs });
      blocks = loaded.blocks;
      activeBlockMs = loaded.activeBlockMs;
    } else {
      blocks = scanBlocks(dataDir, kind, market, effectiveFromMs, toMs);
    }
    ...
    if (blocks.length === 0) {
      ...
      if (cp && cp.pending_block) {
        const pendingIsActive = rawLayout === 'v4' && (cp.pending_block.active === true);
        const horizon = checkFinalizedHorizon(cp.pending_block, finalizedThroughMs, frozenInventory, dataDir, market, kind, pendingIsActive);
        if (horizon.canFinalize) {
          ...
          let pendingBlocks;
          if (rawLayout === 'v4') {
            const loaded = await v4Source.loadBlocks({ fromMs: cp.pending_block.block_start_ms, toMs: cp.pending_block.block_start_ms + BLOCK_DURATION_MS });
            pendingBlocks = loaded.blocks;
          } else {
            pendingBlocks = scanBlocks(dataDir, kind, market, cp.pending_block.block_start_ms, cp.pending_block.block_start_ms + BLOCK_DURATION_MS);
          }
          if (pendingBlocks.length > 0) {
            return processBlocks({ dataDir, market, runId, derivedDir, blocks: pendingBlocks, cp, finalizedThroughMs, frozenInventory, isEofRun: true, kind, rawLayout, v4Source, activeBlockMs: null });
          }
        }
    ...
    return processBlocks({ dataDir, market, runId, derivedDir, blocks, cp, finalizedThroughMs, frozenInventory, isEofRun: false, kind, rawLayout, v4Source, activeBlockMs });
  } finally {
    if (v4Source) {
      try { await v4Source.close(); } catch (_) {}
    }
  }
}
 ...
 */
function processBlocksNonTrade({ dataDir, market, derivedDir, blocks, cp, finalizedThroughMs, frozenInventory, kind, rawLayout = 'v3', v4Source = null, activeBlockMs = null }) {
  let pendingBlock = null;
  let processed = 0, errors = 0;
  const manifestUpdates = [];

  // ── B5: On entry, handle checkpoint pending block for book_updates ──
  const isV4NonTrade = rawLayout === 'v4';
  if (cp?.pending_block && kind !== 'trades') {
    const pendingPath = cp.pending_block.replay_identity?.input_path;
    const pendingV4Block = isV4NonTrade ? v4Source.getBlock(cp.pending_block.block_start_ms) : null;
    const pendingExists = isV4NonTrade ? !!pendingV4Block : (pendingPath && existsSync(pendingPath));
    if (!pendingExists) {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (isV4NonTrade && !candidateBlock.segmentProof) {
      writeQuarantineReport(derivedDir, market, candidateBlock.ms,
        'MISSING_FINALIZED_INPUT', { kind, reason: 'raw-v4-book-window-empty' }, kind);
      writeVerifiedMissingRecord(market, candidateBlock.ms, {
        kind,
        reason: 'raw-v4-book-window-empty',
      }, derivedDir);
      emitStructured({
        level: 'BLOCKED',
        processed,
        ...
        kind,
        market,
        cursor_ms: candidateBlock.ms,
      });
      return { processed, errors, manifestUpdates, blocked: true, blockedReason: 'verified-missing', blockedState: 'verified-missing' };
    }
      ...
      if (candidateBlock.ms !== expectedNext) {
        if (candidateBlock.ms > expectedNext) {
          ...
          pendingBlock = {
            ...
            active: isV4NonTrade ? candidateBlock.active : undefined,
            replay_identity: {
              market,
              block_start_ms: candidateBlock.ms,
              input_path: candidateBlock.fullPath,
              v4_cursor: isV4NonTrade ? candidateBlock.v4Cursor : undefined,
            },
          };
          continue;
        }
  ...
  // ═══ EOF: horizon check ═══
  if (pendingBlock !== null) {
    const pendingIsActive = isV4NonTrade && pendingBlock.block_start_ms === activeBlockMs;
    const horizon = checkFinalizedHorizon(pendingBlock, finalizedThroughMs, frozenInventory, dataDir, market, kind, pendingIsActive);
 ...
 */
function processBlocks({ dataDir, market, runId, derivedDir, blocks, cp, finalizedThroughMs, frozenInventory, isEofRun, kind = 'trades', rawLayout = 'v3', v4Source = null, activeBlockMs = null }) {
  // P0-1: non-trades path uses simplified processing loop
  if (kind !== 'trades') {
    return processBlocksNonTrade({ dataDir, market, derivedDir, blocks, cp, finalizedThroughMs, frozenInventory, kind, rawLayout, v4Source, activeBlockMs });
  }

  const isV4 = rawLayout === 'v4';
  ...
  if (pendingBlock !== null && cp && cp.open_burst) {
    if (isV4) {
      const pendingV4Block = blocks.find(b => b.ms === pendingBlock.block_start_ms);
      if (!pendingV4Block) {
        writeQuarantineReport(derivedDir, market, pendingBlock.block_start_ms,
          'E021: pending block not found in v4 source on restart',
          { expected_ms: pendingBlock.block_start_ms, v4_cursor: pendingBlock.replay_identity?.v4_cursor },
          kind);
        throw new Error(`E021: pending block ${pendingBlock.block_start_ms} not found in v4 source on restart`);
      }
      // For active pending blocks the content is expected to grow; skip hash check.
      // For closed pending blocks, verify immutability.
      if (!pendingBlock.active && pendingV4Block.sha256 !== pendingBlock.trade_input_sha256) {
        writeQuarantineReport(derivedDir, market, pendingBlock.block_start_ms,
          'E021: pending block input changed on restart',
          { expected: pendingBlock.trade_input_sha256, got: pendingV4Block.sha256, path: pendingV4Block.fullPath });
        throw new Error(`E021: pending block ${pendingBlock.block_start_ms} input changed (expected ${pendingBlock.trade_input_sha256}, got ${pendingV4Block.sha256})`);
      }
      detector.feedTrades(pendingV4Block.trades);
      pendingBlock.reordered_input = pendingV4Block.reordered_input;
      pendingBlock.timestamp_inversion_count = pendingV4Block.timestamp_inversion_count;
      log('INFO', market, `re-fed ${pendingV4Block.trades.length} pending block trades to rebuild state`, { rawLayout });
    } else {
  ...
  let skip5minUntilMs = null;
  function tryCommit5min(row, sourceMeta = {}) {
    if (skip5minUntilMs !== null) {
      if (row.ts < skip5minUntilMs) return;
      skip5minUntilMs = null;
    }
    ...
    if (thirtySBuffer.length === 10) {
      const aligned = thirtySBuffer[0].ts % 300000 === 0
        && thirtySBuffer.every((r, i) => r.ts === thirtySBuffer[0].ts + i * 30000);
      if (!aligned) {
        const mismatchIndex = thirtySBuffer.findIndex((r, i) => r.ts !== thirtySBuffer[0].ts + i * 30000);
        const gapStartMs = mismatchIndex > 0
          ? thirtySBuffer[0].ts + mismatchIndex * 30000
          : thirtySBuffer[0].ts;
        const gapEndExclusive = mismatchIndex > 0 ? thirtySBuffer[mismatchIndex].ts : row.ts;
        rollup5minCommitter.recordGap({ startMs: gapStartMs, endMsExclusive: gapEndExclusive });
        emitStructured({
          level: 'GAP_BARRIER',
          market,
          layer: 'features_5min',
          start_ms: gapStartMs,
          end_ms_exclusive: gapEndExclusive,
          reason: 'non-consecutive-30s-input',
        });
        thirtySBuffer.length = 0;
        thirtySProvenance.length = 0;
        skip5minUntilMs = Math.ceil(row.ts / 300000) * 300000;
        return;
      }
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    const content = readBlockContent(candidateBlock);
    ...
    if (pendingBlock !== null) {
      ...
      if (candidateBlock.ms !== expectedNext) {
        ...
        if (candidateBlock.ms > expectedNext) {
          ...
          const rawLookback = isV4
            ? validateV4RawTradeLookback(v4Source, pendingBlock.block_start_ms)
            : validateRawTradeLookback(dataDir, market, pendingBlock.block_start_ms);

          // Read pending N's trades for feature computation
          const nTradesContent = readPendingBlockContent(pendingBlock, rawLayout, v4Source);
          ...
          const tradedNotional30s = isV4
            ? buildV4RawTradedNotionalLookup(v4Source, pendingBlock.block_start_ms)
            : buildRawTradedNotionalLookup(dataDir, market, pendingBlock.block_start_ms);
          const tradeHistory = isV4
            ? buildV4RawTradeHistory(v4Source, pendingBlock.block_start_ms)
            : buildRawTradeHistory(dataDir, market, pendingBlock.block_start_ms);
          ...
          const nRows = computeFeatures1s({
            ...
            bookSnapshot,
            bookEvents: bookSnapshot.events,
            bookStateAt: bookSnapshot.stateAt,
          });
          ...
          const nextPendingInfo = {
            ...
            active: isV4 ? candidateBlock.active : undefined,
            replay_identity: {
              market,
              block_start_ms: candidateBlock.ms,
              input_path: candidateBlock.fullPath,
              v4_cursor: isV4 ? candidateBlock.v4Cursor : undefined,
            },
            reordered_input,
          ...
          const result = committer.commitFinalizedBlock(
            ...
            nRows,
            {
              auxiliary_input_hashes: { ...(rawLookback.hashes || {}), ...(tradeHistory.hashes || {}) },
              ...
              raw_v4_segment_proof: isV4 ? getV4SegmentProof(v4Source, pendingBlock.block_start_ms) : undefined,
              raw_v4_cursor: isV4 ? v4Source.cursor : undefined,
            },
  ...
  if (pendingBlock !== null) {
    const pendingIsActive = isV4 && pendingBlock.block_start_ms === activeBlockMs;
    const horizon = checkFinalizedHorizon(pendingBlock, finalizedThroughMs, frozenInventory, dataDir, market, kind, pendingIsActive);
    ...
    detector.flushAll();
    const finalTradesContent = readPendingBlockContent(pendingBlock, rawLayout, v4Source);
