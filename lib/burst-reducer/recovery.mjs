 ...
 *
 * When kind !== 'trades', only validate the kind-qualified checkpoint
 * (no manifest records to reconcile for book_updates).
 *
 * @param {string} market
 ...
 */
export function reconcileMarketState(market, derivedDir, kind = 'trades') {
  // For non-trades kind, just validate the kind-qualified checkpoint
  // No manifest records to reconcile for book_updates (no intents/commits)
  if (kind !== 'trades') {
    const cp = loadCheckpoint(market, derivedDir, kind);
    if (cp === CHECKPOINT_CORRUPT) {
      process.stderr.write(JSON.stringify({
        ts: new Date().toISOString(),
        level: 'FATAL',
        market,
        msg: `corrupt-checkpoint: ${kind} checkpoint file was empty or invalid JSON, backed up as .bak`,
      }) + '\n');
      return {
        cursor: null,
        generation: 0,
        quarantinedKeys: [],
        errors: [`corrupt-checkpoint: ${kind} checkpoint file was empty or invalid JSON, backed up as .bak`],
