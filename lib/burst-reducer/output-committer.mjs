...
export class OutputCommitter {
  ...
  commitFinalizedBlock(finalizedBlock, nextPendingBlock, nextDetectorState, rows, manifestInputs, checkpointGeneration, commitId, isEofFinalization) {
    ...
    // The committer is the point where a 1s block becomes durable/finalized.
    // Older callers build rows without this envelope flag; stamp it here so
    // downstream retention checks see the same truth as the 30s rollup.
    const committedRows = rows.map((row) => ({
      ...row,
      _quality: { ...row._quality, finalized: row._quality?.finalized ?? true },
    }));
    this._validateRows(committedRows, block_start_ms);

    const date = formatDate(block_start_ms);
    const time = formatBlockTime(block_start_ms);
    const content = committedRows.map(r => JSON.stringify(r)).join('\n') + '\n';
    ...
    const rawV4SegmentProof = manifestInputs.raw_v4_segment_proof || null;
    const rawV4Cursor = manifestInputs.raw_v4_cursor || null;
    ...
    const cp = {
      ...
      ...(rawV4Cursor ? { raw_v4_cursor: rawV4Cursor } : {}),
      ...(rawV4SegmentProof ? { raw_v4_segment_proof: rawV4SegmentProof } : {}),
    };
