...
export class RollupOutputCommitter {
  ...
  reconcileCommitted1s() {
    ...
    for (const [sourceManifestKey, record] of Object.entries(sourceManifest.processed_blocks || {})) {
      ...
      const rows = parseJsonl(sourceContent, record.output_path).map((row) => {
        if (row._quality?.finalized === false) {
          const error = new Error(`E_ROLLUP_CONTRADICTORY_FINALIZED: 1s shard is explicitly unfinalized: ${record.output_path}`);
          error.code = 'E_ROLLUP_CONTRADICTORY_FINALIZED';
          throw error;
        }
        return { ...row, _quality: { ...row._quality, finalized: true } };
      });
      checked += 1;
