...
export class Rollup5minCommitter {
  ...
  recordGap({ startMs, endMsExclusive, reason = 'non-consecutive-30s-input', sourceLayer = FEATURES_30S_DIR }) {
    if (!Number.isFinite(startMs) || !Number.isFinite(endMsExclusive) || endMsExclusive <= startMs) return null;
    const manifest = load5minManifest(this._market, this._derivedDir) || initialManifest(this._market);
    if (!manifest.gaps) manifest.gaps = [];
    const key = `${sourceLayer}:${startMs}:${endMsExclusive}:${reason}`;
    if (!manifest.gaps.some((gap) => gap.key === key)) {
      manifest.gaps.push({
        key,
        layer: FEATURES_5MIN_DIR,
      ...
      manifest.gaps.sort((a, b) => a.start_ms - b.start_ms);
   ...
   * @param {Array<string>} [params.sourceOutputPaths] — all committed 30s source paths
   * @param {Array<string>} [params.sourceManifestKeys] — all committed 30s manifest keys
  ...
  commitWindow({ rows, sourceInputSha256, sourceOutputPath, sourceOutputHash, sourceManifestKey = null, sourceOutputPaths = [], sourceManifestKeys = [] }) {
    ...
    const inputHash = sourceInputSha256 || sourceOutputHash || sha256(JSON.stringify(rows));
      ...
      if (record.output_row_hash && actualHash !== record.output_row_hash) {
        ...
        const normalizedRows = rows.map((row) => {
          ...
          if (!row?._quality || !['features_1s', FEATURES_30S_DIR].includes(sourceLayer)
            ...
            const err = new Error(`E_FIVEMIN_INVALID_SOURCE_PROVENANCE: invalid 30s quality in ${record.output_path}`);
          ...
          _quality: {
            ...row._quality,
            source_layer: FEATURES_30S_DIR,
            finalized: row._quality.finalized,
          },
        };
        });
    ...
    while (i + 9 < thirtySRecords.length) {
      ...
      if (aligned) {
        ...
        try {
          ...
          if (!['E_5MIN_UNALIGNED', 'E_5MIN_MISSING_WINDOW', 'E_5MIN_PARTIAL_WINDOW'].includes(error.code)) {
            if (!firstRecoveryError) firstRecoveryError = error;
          } else {
            this.recordGap({ startMs: bucket[0].ts, endMsExclusive: bucket[bucket.length - 1].ts + 30000, reason: error.code });
          }
        }
      } else {
        this.recordGap({ startMs: bucket[0].ts, endMsExclusive: bucket[bucket.length - 1].ts + 30000, reason: 'non-consecutive-30s-input' });
      }
      i += 10;
    }
