...
export class OkxConnector extends BaseConnector {
  ...
  subscribe() {
    ...
    // 購読 ack の成立を追跡する (観測のみ。復旧の判断には使わない)。M1/M4。
    this.subscribeRequested('trades');
    this.subscribeRequested('books');
    this.subscribeRequested('liquidation-orders');
  }
  ...
  _preprocessRaw(raw) {
    ...
    // 自分から送った ping への応答は生の 'pong' (JSONではない)。握って消費する。
    // 消費しないと 20 秒ごとに JSON.parse エラーが出る (2026-09-23 実測)。
    if (str === 'pong') return true;
    return false;
  }

  /**
   * OKX は「30秒間 data が push されない」と接続を切る。受信ごとに更新されるタイマーの
   * 期限前にアプリ層の `ping` を送る (公式 docs-v5)。※閾値の短縮は別変更 (M5)。
   */
  appHeartbeat() {
    return { intervalMs: 20000, payload: 'ping' };
  }

  _onMessage(data) {
    // 購読 ack の成否を記録する (今までは成功を無視・失敗は emit のみだった)。M4。
    if (data.event === 'subscribe') {
      const channel = data.arg?.channel;
      if (channel) this.subscribeAcked(channel);
      return;
    }
    if (data.event === 'error') {
      const channel = data.arg?.channel || 'unknown';
      this.subscribeFailed(channel, data.msg || data.code || null);
  ...
  _handleDepth(data) {
    ...
    // Issue #12: missing/unparseable exchange ts → null (unknown), never
    // Date.now().  OKX books records always carry ts on real feeds.
    const parsedTs = parseInt(first.ts, 10);
  ...
  _handleTrade(data) {
    ...
    for (const t of trades) {
      ...
      const parsedTs = parseInt(t.ts, 10);
      this._emitTrade(
        parseFloat(t.px),
        qty,
        t.side === 'buy' ? 'buy' : 'sell',
        // Issue #12: missing/unparseable exchange ts → null (unknown).
        Number.isFinite(parsedTs) ? parsedTs : null,
        String(t.tradeId || '')
      );
    }
  }
  ...
  _handleLiquidation(data) {
    ...
    for (const liq of items) {
      ...
      // Issue #12: missing/unparseable exchange ts → null (unknown).
      const parsedLiqTs = parseInt(liq.ts, 10);
      const sourceTs = Number.isFinite(parsedLiqTs) ? parsedLiqTs : null;
  ...
  _replayRingBufAfterSnapshot() {
    for (const msg of this._ringBuf) {
      ...
      const rawBids = first.bids || [];
      const rawAsks = first.asks || [];
      // Issue #12: no Date.now() substitution on replay either — the
      // buffered message's own exchange ts is used (null when missing).
      const parsedReplayTs = parseInt(first.ts, 10);
      const replayTs = Number.isFinite(parsedReplayTs) ? parsedReplayTs : null;
      // Fail-closed (Astra P1): never apply a replayed diff the downstream
      // stream rejected (invalid/missing ts) — doing so would desync this
      // book from the persisted stream and open a downstream sequence gap.
      // On rejection, abandon the replay and re-sync from a fresh snapshot.
      if (!this._emitDepth('update', toPairs(rawBids), toPairs(rawAsks), replayTs, msgSeq, { prev_seq: prevSeqId ?? localSeq }, msg._ingress ?? null)) {
        this._handleSequenceGap(`OKX replay emit rejected (invalid ts=${replayTs}): msgSeq=${msgSeq}, prevSeqId=${prevSeqId ?? 'null'}`, msg);
        return;
      }
  ...
  async _syncBook() {
    ...
    // Success is only real when the sync actually completed: a snapshot was
    // received AND nothing derailed the sync while awaiting it. The waiter is
    // resolved the moment the snapshot frame arrives, but processing continues
    // synchronously — a replay rejection or concurrent sequence gap (Astra P1
    // fix) then clears the book, resets _wsSnapshotReceived and moves to
    // 'reconnecting' with a reconnect timer armed. The resumed _syncBook must
    // not clobber that state back to 'running': reporting a failed sync as
    // success leaves the connector 'running' with an empty book and no
    // snapshot (Astra P2). Same guard covers a REST-fallback restore that
    // landed after a mid-sync reconnect/error took over — leave the state
    // machine to the reconnect/error machinery instead.
