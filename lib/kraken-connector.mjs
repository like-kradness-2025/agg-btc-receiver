...
// Kraken book frames carry no exchange event timestamp: ts is the local
...
export class KrakenSpotConnector extends BaseConnector {
  ...
  _replayRingBufAfterSnapshot() {
    ...
    for (const msg of pending) {
      if (msg?.a || msg?.b) this._handleBookUpdate(msg, msg._ingress ?? null);
    }
  ...
  /**
   * @param {Object} data - parsed book update frame
   * @param {Object|null} [ingress] - ingress snapshot of the buffered message
   *   (ring-buffer replay). Live calls omit it and inherit the current
   *   socket frame's ingress metadata.
   */
  _handleBookUpdate(data, ingress = null) {
  ...
  _handleTrades(msg) {
    ...
    for (const t of trades) {
      ...
      // Issue #12: missing/unparseable exchange time is unknown (null) —
      // never substitute the local receive time (fail-closed drop).
      const rawTime = Number(time);
      const ts = Number.isFinite(rawTime) && rawTime > 0 ? Math.floor(rawTime * 1000) : null;
      this._emitTrade(parseFloat(price), parseFloat(qty), side === 's' ? 'sell' : 'buy', ts, `${time}-${price}-${qty}-${orderType}-${misc}`);
    }
  }
