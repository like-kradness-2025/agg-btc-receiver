...
// Exchange REST timestamps can lead the local wall clock by a small amount.
// Treat only bounded skew as an observation-time issue; larger look-ahead stays
// fail-closed as future_sample.
...
// Keep persisted OI event time at or before the collector receive time. The
// normalized as_of_ms is preferred; source_ts/ts are fallbacks for error rows.
...
export function normalizeOpenInterest(marketOrInput, row, maybeOptions) {
  ...
  const clockSkewToleranceMs = Math.max(
    0,
    finite(options.clockSkewToleranceMs ?? DEFAULT_OI_CLOCK_SKEW_TOLERANCE_MS)
  ...
  const effectiveAsOfMs = futureSkewMs > 0 && futureSkewMs <= clockSkewToleranceMs
    ? nowMs
    : asOfMs;
